...
void create_cyclists(long id){
	cyclists[id].speed = 30; //na primeira volta todos ciclistas tem velocidade de 30km/h
	cyclists[id].speed_last_lap = 60; //iniciando essa variável com um valor que não trará problema
...
void * run_const(void * argument){
	...
	while ((cyclists[team_0.leader].completed_laps < total_laps || cyclists[team_1.leader].completed_laps < total_laps) && !broke){
		...
		if ((cyclists[thread_id].pos % track_size) == cyclists[thread_id].start_pos){
			...
			/***** SORTEAR VELOCIDADE E ATUALIZAR A VARIÁVEL REFERENTE A ÚLTIMA VELOCIDADE ****/
			/**** Verificar se pode colocar a velocidade sorteada ou tem que ir mais devagar ****/
